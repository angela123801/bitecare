...
The one-time registration step, called immediately after sign-up.
  - A Resident choice is ALWAYS accepted, so ordinary public sign-up keeps
    working whether or not open registration is enabled.
  - A privileged role is accepted ONLY while open registration is enabled.
  - Runs ONCE per account (rejected if `role_selected_at` is already set), so a
    signed-in user cannot use it to change their own role afterwards.
  - Validates the role, writes it to the profile AND to the sign-in token's
    app_metadata, and records an audit entry. No secrets are logged.
...
BEGIN
  ...
  -- Residents may always self-register. Privileged roles require the open
  -- registration switch to be on.
  IF p_role <> 'user' AND NOT public.is_open_registration_enabled() THEN
